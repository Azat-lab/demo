...
import java.util.Optional;
    ...
    class HandbookRestController {
        ...
        public ResponseEntity<HttpStatus> deleteOperator(@PathVariable("id") Long operatorId) {
            try {
                countryRepository.deleteById(operatorId);
                return new ResponseEntity<>(HttpStatus.GONE);
            } catch (Exception e) {
                return new ResponseEntity<>(HttpStatus.GATEWAY_TIMEOUT);
            }
        ...
        public ResponseEntity<Operator> getOperatorByCode(@PathVariable("id") Integer operatorCode) {
            Optional<Operator> operatorData = operatorRepository.findByCode(operatorCode);
            if (operatorData.isPresent()) {
                return new ResponseEntity<>(operatorData.get(), HttpStatus.OK);
            } else {
                return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        ...
        public String saveOperatorCountry(@RequestBody Operator operator) {
            operatorRepository.saveOperator(operator.getOperatorCountry());
